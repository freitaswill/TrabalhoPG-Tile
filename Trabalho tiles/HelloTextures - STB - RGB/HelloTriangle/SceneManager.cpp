...
void SceneManager::setupScene()
{
	...
	float slice_x = 1.0 / 8.0;
	float slice_y = 1.0 / 2.0;
...
void SceneManager::setupTexture()
{
	...
	unsigned char *data = stbi_load("../textures/lena.png", &width, &height, &nrChannels, 0);

	cout << width << " " << height << " " << nrChannels << endl;
	
	if (data)
	{
		//char corModificadora[3]; corModificadora[0] = 255; corModificadora[1] = 0; corModificadora[2] = 255;
		for (int i = 0; i < width * height * nrChannels; i+=nrChannels)
		{
			...
			data[i + 2] = data[i + 2] ^ 255; sepia*/

			/*data[i] = data[i] | corModificadora[0];
			data[i + 1] = data[i + 1] | corModificadora[1];
			data[i + 2] = data[i + 2] | corModificadora[2];*/

			//if (data[i] > 1 && data[i + 1]  < 100 && data[i+2] < 100) {
			//	data[i] = 30; //R
			//	data[i + 1] = 150; //G
			//	data[i + 2] = 30; //B
			//}
